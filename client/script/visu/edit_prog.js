function EditProg() {
            ...
            if (this.last_sc === this.t1.sc && this.last_sr === this.t1.sr) {
                switch (this.t1.sc) {
                    case this.ROW.DESCRIPTION:
                        ...
                        break;
                    case this.ROW.INTERVAL_MIN:
                        var self = this;
                        vtime_edit.prep(this.data[this.t1.sr].interval_min, 0, INT32_MAX, self, this.t1.sc, 301);
                        showV(vtime_edit);
                        break;
                    case this.ROW.MAX_ROWS:
                        var self = this;
                        vint_edit.prep(this.data[this.t1.sr].max_rows, 0, INT32_MAX, self, this.t1.sc, 302);
                        showV(vint_edit);
                        break;
                }
            }
        ...
        try {
            switch (k) {
                case this.ROW.DESCRIPTION:
                    ...
                    break;
                case this.ROW.INTERVAL_MIN:
                    this.data[this.t1.sr].interval_min = d;
                    this.t1.updateCell(this.t1.sr, this.t1.sc, intToTimeStr(this.data[this.t1.sr].interval_min));
                    break;
                case this.ROW.MAX_ROWS:
                    this.data[this.t1.sr].max_rows = d;
                    this.t1.updateCell(this.t1.sr, this.t1.sc, this.data[this.t1.sr].max_rows);
                    this.logTableSize();
                    break;
                default:
    ...
    this.dataRowChanged = function (i) {
        if (
               ...
               this.data[i].interval_min !== this.data_ini[i].interval_min || 
               this.data[i].max_rows !== this.data_ini[i].max_rows
                ) {
            return true;
        }
        return false;
    ...
    this.getTableSizeMB = function () {
        var size = 0;
        for (var i = 0; i < this.data.length; i++) {
            size += this.data[i].max_rows * this.ROW_SIZE;
        }
        return size / 1000000;
    };
    this.logTableSize = function () {
        var size = this.getTableSizeMB();
        logger.ntfs(trans.get(303) + size.toFixed(1) + trans.get(304));
    };
                ...
                case this.ACTION.GET:
                    ...
                    for (i = 0; i < d.length; i++) {
                        this.data.push({
                            ...
                            interval_min: parseInt(d[i].interval_min),
                            max_rows: parseInt(d[i].max_rows)
                        });
                        this.data_ini.push({
                            id: parseInt(d[i].id),
                            description: d[i].description,
        ...
        try {
            ...
            for (var i = 0; i < this.data.length; i++) {
                this.t1.appendRow([
                    ...
                    intToTimeStr(this.data[i].interval_min),
                    this.data[i].max_rows
                ]);
            }
        } catch (e) {
